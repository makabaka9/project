...
import { queryPisStatus, queryAuxiliaryStatusData, queryFaultPie, queryFaultInfo, queryFaultHistogram } from './service';
...
const Model: ModelType = {
  ...
  effects: {
    ...
    //   });
    // },
    *fetchPisStatus({ payload }, { call, put }) {
      const response = yield call(queryPisStatus, payload);
      yield put({
        type: 'pisStatus',
        payload: response,
      });
  ...
  reducers: {
    pisStatus(state, action) {
      return {
        ...state,
        pisStatusData: action.payload,
      };
    },
