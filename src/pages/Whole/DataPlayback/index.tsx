...
interface TrainProps {
  loading: boolean;
  dispatch: Dispatch;
  wholeAndDataPlayback: StateType;
}

const Train: React.FC<TrainProps> = (props) => {
  const { wholeAndDataPlayback, loading, dispatch } = props;
  const { } = wholeAndDataPlayback;
  ...
  useEffect(() => {
    dispatch({
      type: 'wholeAndDataPlayback/fetchBasicInfo',
      payload: trainCode,
    });
  }, []);
  useEffect(() => {
    dispatch({
      type: 'wholeAndDataPlayback/fetchReliability',
      payload: trainCode,
    });
  }, []);
  useEffect(() => {
    dispatch({
      type: 'wholeAndDataPlayback/fetchComfort',
      payload: trainCode,
    });
  }, []);
  useEffect(() => {
    dispatch({
      type: 'wholeAndDataPlayback/fetchStability',
      payload: trainCode,
    });
  }, []);
...
export default connect(
  ({
    ...
    wholeAndDataPlayback,
    // loading: loading.effects['wholeAndDataPlayback/fetchBasic'],
    loading: loading.effects['wholeAndDataPlayback/fetchBasicInfo'],
  }),
)(Train);
